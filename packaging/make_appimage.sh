#!/usr/bin/env bash
# Build a Qt6 Linux AppImage for the Bandai Design Master emulator.
# Requires Qt6 qmake, a compiler, linuxdeploy, and linuxdeploy-plugin-qt. The
# tools are downloaded into tools/appimage on first use unless LINUXDEPLOY and
# LINUXDEPLOY_PLUGIN_QT point to existing executables.
set -euo pipefail
...
( cd "$BUILD" && "$QMAKE" "$ROOT/BDMEmu.pro" CONFIG+=release && make -j"$(getconf _NPROCESSORS_ONLN 2>/dev/null || echo 2)" )
[ -x "$BUILD/$APP" ] || die "build did not produce $BUILD/$APP"

rm -rf "$APPDIR"
install -Dm755 "$BUILD/$APP" "$APPDIR/usr/bin/$APP"
install -Dm644 "$HERE/bdmemu.desktop" "$APPDIR/usr/share/applications/bdmemu.desktop"
install -Dm644 "$HERE/bdmemu.svg" "$APPDIR/usr/share/icons/hicolor/scalable/apps/bdmemu.svg"
cp "$HERE/bdmemu.svg" "$APPDIR/bdmemu.svg"
...
"$LINUXDEPLOY" \
    ...
    --desktop-file "$APPDIR/usr/share/applications/bdmemu.desktop" \
    --icon-file "$HERE/bdmemu.svg" \
    --plugin qt \
    --output appimage
